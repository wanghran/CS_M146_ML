...
def cv_performance(clf, X, y, kf, metric="accuracy"):
    ...
    for train, test in kf:
        ...
        performance(y[test], clf.decision_function(X[test]), metric=metric)
        score.append(performance(y[test], clf.decision_function(X[test]), metric=metric))
...
def select_param_linear(X, y, kf, metric="accuracy"):
    ...
    for c in C_range:
        ...
        score.append(cv_performance(clf, X, y, kf, metric))
    return C_range[score.index(max(score))]
    ### ========== TODO : END ========== ###


def score_plot(X, y, kf):
    C_range = 10.0 ** np.arange(-3, 3)
    score = np.zeros((len(C_range), 3))
    i = 0
    for c in C_range:
        clf = SVC(kernel='linear', C=c)
        score[i][0] = cv_performance(clf, X, y, kf, metric="accuracy")
        score[i][1] = cv_performance(clf, X, y, kf, metric="f1_score")
        score[i][2] = cv_performance(clf, X, y, kf, metric="auroc")
        i += 1
    return score
