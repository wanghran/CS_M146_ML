...
class Cluster(object) :
    ...
    def centroid(self) :
        ...
        pos = []
        for point in col:
            pos.append(point.attrs)
            if point.label in common_label:
                common_label[point.label] += 1
            else:
                common_label[point.label] = 1
        centroid = Point("centroid", max(common_label, key=common_label.get), np.mean(pos, axis=0))
        return centroid
        ### ========== TODO : END ========== ###
