...
class PolynomialRegression() :
    ...
    def generate_polynomial_features(self, X) :
        ...
        for i in range(m):
            Pho = Phi[:, 1:2] ** (i+2)
            Phi = np.concatenate((Phi, Pho), axis=1)
        print(Phi)
    ...
    def fit_GD(self, X, y, eta=None,
        ...
        for t in xrange(tmax) :
            ...
            self.coef_ -= 2 * eta * (np.sum((np.dot(self.coef_, X.T) - y)*X.T, axis=1))
            # track error
...
def main() :
    ...
    for i in range(11):
        ...
        error1 = model.rms_error(train_data.X, train_data.y)
        error_list_train.append(error1)
    for j in range(11):
        model = PolynomialRegression(m=j)
        model.fit(train_data.X, train_data.y)
        error2 = model.rms_error(test_data.X, test_data.y)
        error_list_test.append(error2)
    plt.plot(np.arange(11) + 1, error_list_train, label="train")
    plt.plot(np.arange(11) + 1, error_list_test, label="test")
    plt.legend()
    ...
    # print error_list_train.index(min(error_list_train))
    # print error_list_test.index(min(error_list_test))
    ### ========== TODO : END ========== ###
